...
export default async function MoviePage({params}: Props) {
    const {id, lang} = await params
    try {
        const {movie, credits, videos, images, providers, recommendations} = await getMovie(Number(id), lang)
        const crew = credits.crew?.sort((a) => a.role.toLowerCase() === "screenplay" ? 1 : -1).map(serializePeople)
...
function movieJsonLd(movie: any, cast: any[], crew: any[]) {
    const directors = crew.filter((a: any) => a.role.toLowerCase() === "director")
